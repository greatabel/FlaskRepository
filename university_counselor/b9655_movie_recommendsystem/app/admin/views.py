...
def login():
    ...
    if form.validate_on_submit():
        ...
        session["admin"] = data[
            "account"
        ]  # 如果密码正确,登录成功后添加保持会话,保存管理员账号 # session['admin'] = request.form['account']
        session["admin_id"] = admin.id  # 保存管理员ID(用作日志)
        # 将登录操作添加到管理员登录日志列表
        adminlog = Adminlog(admin_id=session["admin_id"], ip=request.remote_addr)
...
def tag_del(id=None):
    ...
    # 添加删除标签操作到操作日志列表
    oplog = Oplog(
        admin_id=session["admin_id"], ip=request.remote_addr, reason="删除标签:" + tag.name
    )
    db.session.add(oplog)
    db.session.commit()
...
def movie_list(page=None):
    ...
    page_data = (
        ...
        .filter(Tag.id == Movie.tag_id)  # .join(Tag):因为要通过标签id查询标签名称;filter():关联连个模型
...
def movie_edit(id=None):
    ...
    return render_template(
        "admin/movie_edit.html", form=form, movie=movie
    )  # 传movie是为了赋予初值,因为是修改电影,标签上面会显示之前的电影信息
...
def user_list(page=None):
    if page == None:
        page = 1
    page_data = User.query.order_by(User.addtime.desc()).paginate(
        page=page, per_page=10
    )
...
def user_del(id=None):
    ...
    # 将删除会员操作保存到操作日志列表
    oplog = Oplog(
        admin_id=session["admin_id"], ip=request.remote_addr, reason="删除会员:" + user.name
    )
    db.session.add(oplog)
    db.session.commit()
    ...
    if request.method == "GET":
        if role.auths == "":  # 有的管理员没有权限就是空的,就不返回他的权限列表了
            ...
            form.auths.data = list(
                map(lambda v: int(v), role.auths.split(","))
            )  # 将字符串'1,2,3'转化成[1,2,3]
    if form.validate_on_submit():
        data = form.data
        role_count = Role.query.filter_by(name=data["name"]).count()
