...
def img():
    ...
    try:
        ...
        if (
            group_list.groups[group_id].visit
            == group_list.groups[group_id].real - group_list.groups[group_id].die
        ):  # 控制访问
            group_list.groups[group_id]._refresh()  # 更新柱子参数
            for i in range(5 - group_list.groups[group_id].real):  # 机器人
                live, ac = (
                    group_list.groups[group_id]
                    .robmodel[i]
                    .detect(group_list.groups[group_id].action[5])
                )
            ...
            group_list.groups[group_id].isok = [
                True,
                True,
                True,
                True,
...
def getact():
    ...
    if len(group_list.groups) > 0:
        if group_list.groups[group_id].isok[bird_id]:
            ...
            print('$'*20, group_list.groups[group_id].action)
...
def abel_refresh():
    ...
    # 初始化对战组
    group_listA = Group_list()
    # group_listA.groups[divide_userid].isdie = False
    # 初始化连接器
    ConA = Connect(group_listA)
    if divide_userid <= len(group_list_Arr):
        group_list_Arr[divide_userid] = group_listA
        Con_Arr[divide_userid] = ConA
        ConA.wait_con(divide_userid)
    return str([])
