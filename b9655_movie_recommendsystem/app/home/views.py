...
def index(page=None):
    ...
    tags = Tag.query.all()
    page_data = Movie.query.order_by(Movie.addtime.desc())
    # 1.电影标签(国产,动漫,科技...)
    ...
    if int(time) != 0:
        if int(time) == 1:  # 降序(最近)
            ...
            page_data = Movie.query.order_by(Movie.addtime.asc())
    ...
    if int(pm) != 0:
        if int(pm) == 1:
            ...
            page_data = Movie.query.order_by(Movie.playnum.asc())
    ...
    if int(cm) != 0:
        if int(cm) == 1:
            ...
            page_data = Movie.query.order_by(Movie.commentnum.asc())
...
def loginlog(page=None):
    if page == None:
        page = 1
    page_data = (
        Userlog.query.filter_by(user_id=session["user_id"])
...
def moviecol(page=None):
    ...
    page_data = (
        ...
        .filter(Moviecol.user_id == session["user_id"])
...
def search(page=None):
    ...
    # 电影信息
    page_data = (
        Movie.query.filter(Movie.title.ilike("%" + key + "%"))  # 模糊匹配
        .order_by(Movie.addtime.desc())
        .paginate(page=page, per_page=4)
    )
    # 搜索个数
    movie_count = Movie.query.filter(Movie.title.ilike("%" + key + "%")).count()  # 模糊匹配
    page_data.key = key  # 把搜索的关键字key传入翻译的模板ui/search.html

    return render_template(
        "home/search.html", key=key, page_data=page_data, movie_count=movie_count
    )
...
def play(id=None, page=None):
    ...
    page_data = (
        Comment.query.join(User)
        .filter(User.id == Comment.user_id, Comment.movie_id == id)
        .order_by(Comment.addtime.desc())
        .paginate(page=page, per_page=6)
    )
    # 提交评论
    if "user" in session and form.validate_on_submit():
        data = form.data
        comment = Comment(
            content=data["content"], movie_id=id, user_id=session["user_id"]
        )
    ...
    return render_template(
        "home/play.html", movie=movie, tag=tag, form=form, page_data=page_data
    )
