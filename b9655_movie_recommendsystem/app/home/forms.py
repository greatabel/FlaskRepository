...
class RegistForm(FlaskForm):
    ...
    email = StringField(
        label="邮箱",
        validators=[DataRequired("请输入邮箱!"), Email("邮箱格式不正确!")],  # Email模块验证邮箱格式
        description="邮箱",
        render_kw={
            "class": "form-control input-lg",
            "placeholder": "邮箱",
    ...
    phone = StringField(
        ...
        validators=[
            DataRequired("请输入手机号!"),
            Regexp(
                "1[3458]\\d{9}", message="手机格式不正确!"
            ),  # Regexp正则模块验证手机格式(第一位:1;第二位:3或4或5或8;加上后面9位数)
        ],
        description="手机",
        render_kw={
            "class": "form-control input-lg",
    ...
    repwd = PasswordField(
        label="确认密码",
        validators=[DataRequired("请输入确认密码!"), EqualTo("pwd", "两次密码输入不一致!")],
        description="确认密码",
        render_kw={
            "class": "form-control input-lg",
            "placeholder": "确认密码",
...
class LoginForm(FlaskForm):
    ...
    submit = SubmitField("登录", render_kw={"class": "btn btn-lg btn-success btn-block"})
...
class UserdetailForm(FlaskForm):
    ...
    info = TextAreaField(
        label="简介", description="简介", render_kw={"class": "form-control", "rows": 10}
    )
